...
class FightTest(Test):
    ...
    def _load(self):
        def set_scene():
            self._player = TestCharacter(position=(0, -0.1), parent=Test.screen)
            self._player.apply_effect(Game.content_manager.get_effect("Stun"))
            self._player.scale = (0.15, 0.15)
            self._player.swap_weapon(Knife())

            self._enemy = TestEnemy(position=(0, 0.1), parent=Test.screen)
            self._enemy.apply_effect(Game.content_manager.get_effect("Stun"))
            self._enemy.scale = (0.15, 0.15)
